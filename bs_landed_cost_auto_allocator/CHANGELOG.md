...
- **Automated tests**: written
  ...
  environment). Run `--test-enable -i bs_landed_cost_auto_allocator` before
...
- **"Why Choose ERP23" listing icons**: the six standard `.webp` badge
  ...
  pattern already used in `bs_lot_recall_report`, `mrp_variant_bom_manager`
...
- Added `static/src/scss/landed_cost_auto_allocator.scss` (registered in
  `web.assets_backend`, same pattern as `bs_lot_recall_report`) with the
