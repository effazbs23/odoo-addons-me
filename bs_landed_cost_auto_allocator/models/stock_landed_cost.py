...
class StockLandedCost(models.Model):
    ...
    def _apply_allocation_rules(self):
        ...
        vendor = self.vendor_bill_id.partner_id or self.picking_ids.partner_id[:1]
        for line in self.cost_lines:
            rule = Rule._find_matching_rule(vendor.id, line.product_id.id)
            if rule:
