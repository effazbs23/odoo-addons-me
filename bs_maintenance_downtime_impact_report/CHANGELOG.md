...
- Added `icon.png` and `main_screenshot.png` (used as the top
  listing-page banner) once supplied in the source assets folder.
  `banner.gif` is mapped via the manifest's `images` key (Apps Store
  carousel preview), not used inside index.html.
