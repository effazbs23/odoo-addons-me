    ...
    def _visible_menu_ids(self, debug=False):
        ...
        if user.has_group('bs_simple_invoice.simple_invoicing_group') \
            ...
            for xmlid in HIDDEN_MENU_XMLIDS:
                res_id = data._xmlid_to_res_id(xmlid, raise_if_not_found=False)
                if res_id:
                    hidden_ids.add(res_id)
                else:
                    _logger.warning(
                        "bs_simple_invoice: menu xmlid '%s' not found, it will "
                        "stay visible to Simple Invoicing users instead of "
                        "being hidden", xmlid,
                    )
