...
## Next step
Naming and pricing confirmed. A full production-readiness audit (60.audit.md) found and fixed one CRITICAL bug: gateway_config/event_template lookups inside dispatch ran in the triggering user's own security context instead of sudo() -- since bs.notify.gateway.config is (correctly) admin-only, any non-admin user's action_confirm/button_validate/_post silently sent zero notifications with no log row at all. Fixed with sudo() on both lookups, verified live with a real non-admin user, and covered by a new regression test (TestNonAdminUserDispatch). Also added index=True to source_record_ref (hot path on every trigger + every smart button). 18/18 tests green. One open (non-blocking) note: manifest category 'Sales/Communication' is not a real Odoo category (spec section 11 asked for it verbatim, but no such category exists in core -- Odoo will silently auto-create an orphan one). Left as-is pending user call since the spec explicitly requested it. Module is otherwise ready for PR: squash per bs23-commit-guidelines before opening against development. Test env note: notify_test_db (local) now has an l10n_bd chart of accounts installed manually via `odoo-bin shell` (core had no journal for the company otherwise) -- this was a one-off local test-DB fix, not a module change.
